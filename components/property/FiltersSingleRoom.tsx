...
import GuestNumberInput from '../SearchBar/GuestsInput';
...
export default function FiltersSingleRoom({
  ...
  return (
    <View className={`flex-1 rounded-xl ${showBookingDetails ? 'p-6 bg-[#161616]' : ''}`}>
      <View className='grid md:flex grid-cols-3 min-[400px]:space-x-4 px-7 py-7 max-md:px-4 max-[400px]:py-4 items-center mb-6 rounded-2xl bg-[#1F1F1F] relative border border-[#323232]'>
        <DatePicker
          value={dateRange}
          minDate={new Date()}
        ...
        <>
          {!isAuthenticated ? (
            <View className='py-4 text-center'>
              ...
              <Link
                to={`${RoutePaths.SIGN_IN}?redirect_to=${encodeURIComponent(window.location.href)}`}
            ...
            <button
              onClick={onBookNow}
              className={`w-full text-white transition-colors rounded-[14px] text-lg 2xl:text-xl font-medium disabled:text-white/40 ${isLoading || !selectedRate ? 'border-2 border-white/40 cursor-default py-1.5' : '!py-2 border-gradient-primary before:rounded-[14px]'}`}
          ...
          {isLoading ? (
            ...
            <View className='mt-6 space-y-2'>
              <View className='overflow-y-auto space-y-2 max-h-32 no-scrollbar'>
                <View className='flex justify-between'>
                  <Text className='text-[#A9A9A9]'>base price</Text>
                  <Text>
                    {selectedRate.payment_options?.payment_types?.[0]?.show_amount_with_commission || selectedRate.payment_options?.payment_types?.[0]?.show_amount}{' '}
                    {selectedRate.payment_options?.payment_types?.[0]?.show_currency_code}
                  </Text>
                </View>
                {[
                  ...(selectedRate.payment_options?.payment_types?.[0]?.tax_data?.taxes || []),
                ].map((tax, index) => (
              ...
              <View className='flex justify-between font-semibold'>
                ...
                <Text>
                  {(selectedRate.payment_options?.payment_types?.[0]?.show_amount_with_commission || selectedRate.payment_options?.payment_types?.[0]?.show_amount).toFixed(2)}{' '}
                  {selectedRate.payment_options?.payment_types?.[0]?.show_currency_code}
                </Text>
              </View>
            </View>
