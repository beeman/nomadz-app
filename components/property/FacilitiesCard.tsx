  ...
  return (
    <View className="container">
      <View className='flex bg-[#121212] border border-[#242424] rounded-[14px] p-4'>
        <View className={`relative flex flex-wrap gap-x-4 gap-y-2 text-white items-center @[270px]:flex-row max-h-12 overflow-hidden ${className}`}>
          {amenities.map((amenity, index) => {
            ...
            return (
              <View key={index} className="flex items-center gap-1 text-xs text-white">
                <Text className="flex-shrink-0 size-4 @[350px]:size-5" style={{display: 'inline-flex', alignItems: 'center'}}><IconComponent /></Text>
