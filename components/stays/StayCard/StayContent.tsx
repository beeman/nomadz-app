...
export function StayContent({ stay }: StayContentProps) {
  ...
  return (
    <View className={`@container flex flex-col justify-between h-full px-4 ${contentPadding} @[330px]:py-6`}>
      <View className="flex justify-between items-start space-x-2">
        <View 
          ref={titleRef}
      ...
      <View className='flex gap-2 justify-between items-center'>
        ...
        {!!lowestPrice && <View className="flex gap-1 justify-end items-center w-full text-white">
          <View className="font-semibold text-xs @[232px]:text-sm @[330px]:text-base @[420px]:text-lg">${formatPrice(lowestPrice)}</View>
          <View className="text-[#A9A9A9] text-[10px] @[232px]:text-xs @[330px]:text-base text-right text-nowrap"> / night</View>
        </View>}
      </View>
    </View>
  );
