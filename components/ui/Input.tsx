...
const Input: React.FC<InputProps> = ({ 
  ...
  const inputClasses = "w-full bg-[#1B1B1B] border-[0.8px] border-[#2F2F2F] rounded-lg px-4 py-2 text-white focus:outline-none focus:border-white/60";
  ...
  return (
    <div className="gap-y-2.5">
      {
        label &&
        <label className={`block text-[#CDCDCD] text-sm ${labelClassName}`}>
          {label}
        </label>
      }
      ...
      {multiline ? (
        <div className="relative">
          ...
          {maxLength && showCounter && (
            value.length >= maxLength ? (
              <span className="flex absolute right-2 bottom-3 gap-1 items-center text-xs pointer-events-none select-none text-alert-red">
                <InfoCircleIcon className="w-4 h-4" />
                Max {maxLength} characters
              </span>
            ) : (
              <span className="absolute right-2 bottom-2 text-xs text-[#CDCDCD] select-none pointer-events-none">
        ...
        <div className="relative">
          {prefix && (
            <span className="absolute left-4 top-1/2 text-white -translate-y-1/2">
              {prefix}
            </span>
          )}
          <input
