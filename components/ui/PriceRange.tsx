...
export default function PriceRange({ 
  ...
  const [range, setRange] = useState<RangeValues>({ min: initialRange?.min || minPrice, max: initialRange?.max || maxPrice });
  const [inputValues, setInputValues] = useState<InputValues>({ min: range.min, max: range.max });
  
  const updateRangeAndNotify = useCallback((newRange: RangeValues, maxPricePresent: boolean = true) => {
  ...
  const calculatePrice = useCallback((clientX: number): number => {
  ...
  return (
    <div className="w-full">
      {/* Distribution Graph */}
      <div className="relative h-16 mb-4" ref={containerRef}>
        <div className="absolute inset-0 flex items-end justify-between gap-0.5">
          {Object.entries(distribution).map(([price, count]) => {
            ...
            return (
              <div
                key={price}
                className={`w-full ${getBarColor(Number(price))}`}
                style={{ height: `${height}%` }}
              />
            );
          })}
        ...
        >
          <div className="size-[18px] bg-white border-2 border-[#58F0B8] rounded-full shadow-lg"/>
        </div>
        <div 
          className="absolute top-auto -ml-3 cursor-pointer -bottom-2.5"
      ...
      <div className="flex justify-between">
        <div className="text-center">
          <div className="mb-0.5 text-xs text-[#ffffffa2]">min</div>
        ...
        <div className="text-center">
          <div className="mb-0.5 text-xs text-[#ffffffa2]">max</div>
          <div className="relative">
            <input
              type="string"
